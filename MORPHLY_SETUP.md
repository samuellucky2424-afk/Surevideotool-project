...
3. Enter these variables directly in the backend environment (locally, `app/.env`; in production, the hosting environment settings):
   ...
   When origin overrides are unset, the API accepts only the shipped website `https://surevideotool-project.vercel.app` and desktop `http://127.0.0.1:47831`. For local development or a custom website, set `APP_ORIGIN` to that exact origin; this replaces the website default. `DESKTOP_APP_ORIGIN` can override the fixed loopback default. Both actual origins must also be allowed on the Morphly key. Existing `SUPABASE_URL` and `SUPABASE_SERVICE_KEY` (or `SUPABASE_SERVICE_ROLE_KEY`) remain required on the backend. Do not prefix server secrets with `VITE_`.
